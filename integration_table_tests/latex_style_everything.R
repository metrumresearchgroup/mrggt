...
tbl_gt <-
  ...
  cols_label(.list = list(grpname = 'Fruit',
                          count = 'Total',
                          color = 'Color')) %>%
  tab_source_note(c('Source code: /integration_table_tests/latex_style_everything.R',
                    'Source code: /integration_table_tests/latex_style_cell_color.tex')) %>%
  as_latex()

readr::write_file(tbl_gt, 'integration_table_tests/latex_style_everything.tex')
